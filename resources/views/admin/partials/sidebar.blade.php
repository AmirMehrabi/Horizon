...
<div class="space-y-1">
    ...
    <!-- Billing System -->
    <div class="relative">
        <button id="billing-menu-toggle" class="flex items-center w-full px-3 py-2 text-sm font-medium text-blue-100 hover:bg-blue-700 hover:text-white rounded-lg transition-colors {{ request()->routeIs('admin.billing.*') ? 'bg-blue-700 text-white' : '' }}">
            <svg class="w-5 h-5 {{ $isRtl ? 'ml-3' : 'mr-3' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
            </svg>
            <span class="flex-1 text-{{ $isRtl ? 'right' : 'left' }}">سیستم صورتحساب</span>
            <svg id="billing-menu-arrow" class="w-4 h-4 transition-transform {{ request()->routeIs('admin.billing.*') ? 'rotate-90' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $isRtl ? 'M15 19l-7-7 7-7' : 'M9 5l7 7-7 7' }}"></path>
            </svg>
        </button>
        
        <!-- Billing Submenu -->
        <div id="billing-submenu" class="mt-1 space-y-1 {{ $isRtl ? 'mr-6' : 'ml-6' }} {{ request()->routeIs('admin.billing.*') ? '' : 'hidden' }}">
            <!-- Dashboard -->
...
document.addEventListener('DOMContentLoaded', function() {
    const billingToggle = document.getElementById('billing-menu-toggle');
    const billingSubmenu = document.getElementById('billing-submenu');
    const billingArrow = document.getElementById('billing-menu-arrow');
    ...
    if (billingToggle && billingSubmenu && billingArrow) {
        billingToggle.addEventListener('click', function(e) {
            ...
            billingSubmenu.classList.toggle('hidden');
            
            // Rotate arrow
            billingArrow.classList.toggle('rotate-90');
            
            // Store state in localStorage
            const isOpen = !billingSubmenu.classList.contains('hidden');
            localStorage.setItem('billing-menu-open', isOpen);
        });
        
        // Restore state from localStorage on page load
        const savedState = localStorage.getItem('billing-menu-open');
        if (savedState === 'true') {
            billingSubmenu.classList.remove('hidden');
            billingArrow.classList.add('rotate-90');
        }
    }
});
</script>
