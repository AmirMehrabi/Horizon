    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <form method="POST" action="{{ route('customer.profile.change-password.update') }}" class="space-y-6 max-w-md">
            ...
            <div>
                ...
                <input type="password" id="current_password" name="current_password" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500 @error('current_password') border-red-500 @enderror">
                @error('current_password')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">اگر رمز عبور تنظیم نکرده‌اید، این فیلد را خالی بگذارید</p>
            </div>
